...
router.get('/', (req: AuthRequest, res) => {
  try {
    ...
    let interventions = [...database.interventions];
    ...
    const interventionsWithCustomers = interventions.map(intervention => {
      const customer = database.customers.find(c => c.id === intervention.customer_id);
      return {
        ...intervention,
        customer: customer ? {
          id: customer.id,
...
router.put('/:id', (req: AuthRequest, res) => {
  try {
    ...
    const intervention = database.interventions.find(i => i.id === id);
    if (!intervention) {
    ...
    if (completed_date) intervention.completed_date = new Date(completed_date);
...
router.get('/alerts/recent', (req: AuthRequest, res) => {
  try {
