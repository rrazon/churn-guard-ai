...
function broadcastHealthScoreUpdates() {
    const customersToUpdate = database_1.database.customers
    ...
    customersToUpdate.forEach(customer => {
        ...
        if (Math.abs(change) > 2) { // Only broadcast significant changes
            ...
            else if (customer.health_score >= 50)
                customer.churn_risk_level = 'medium';
            else if (customer.health_score >= 20)
...
function broadcastAlertNotifications() {
    ...
    if (Math.random() < 0.3) { // 30% chance of generating an alert
        ...
        const customer = database_1.database.customers[Math.floor(Math.random() * database_1.database.customers.length)];
        const alert = {
            type: 'new_alert',
...
function broadcastDashboardUpdates() {
    ...
    const atRiskCustomers = database_1.database.customers.filter(c => c.churn_risk_level === 'high' || c.churn_risk_level === 'critical').length;
    const avgHealthScore = Math.round(database_1.database.customers.reduce((sum, c) => sum + c.health_score, 0) / totalCustomers);
    const totalARR = database_1.database.customers.reduce((sum, c) => sum + (c.mrr * 12), 0);
...
function broadcastToSubscribers(channel, data) {
    ...
    clients.forEach(client => {
        if (client.subscriptions.includes(channel) && client.ws.readyState === ws_1.WebSocket.OPEN) {
            try {
...
function generateAlertMessage(alertType, companyName) {
    switch (alertType) {
...
function triggerTestNotification(type, customerId) {
    const customer = customerId
        ? database_1.database.customers.find(c => c.id === customerId)
        : database_1.database.customers[0];
    if (customer) {
